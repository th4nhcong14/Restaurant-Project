...
<template>
    ...
    <div class="add"> 
        <div class="add-form">
            <h1 style="font-weight: 1000;">Add IMac</h1>
...
export default {
    ...
    methods: {
        ...
        async addProduct() {
            let result = await axios.post("http://localhost:3000/product-imac", {
                name: this.name,
                price: this.price,
                code: this.code,
                // image: this.image
    ...
    async mounted() {
        ...
        let result = await axios.get("http://localhost:3000/product-imac");
        console.warn(result)
